...
export function parsePaints(paints: Paint[] = []) {
  if (!paints) return;
  if (paints.length === 1) {
...
export function parsePaint(paint: Paint) {
  if (!paint) return;
...
export function parseSolid(paint: Paint) {
...
export function parseSolidAsLinearGradient(paint: Paint) {
  if (!paint || paint.visible === false) return;
  return `linear-gradient(${parseColor(
    paint.color,
    paint.opacity
  )},${parseColor(paint.color, paint.opacity)})`;
}

export function parseLinearGradient(paint: Paint) {
  if (!paint || paint.visible === false) return;
  ...
  const stops = paint.gradientStops
    .map((stop) => {
      return `${parseColor(stop.color, paint.opacity)} ${Math.round(
        stop.position * 100
      )}%`;
    })
    .join(", ");
...
export function parseRadialGradient(paint: Paint) {
  if (!paint || paint.visible === false) return;
  const stops = paint.gradientStops
    .map((stop) => {
      return `${parseColor(stop.color, paint.opacity)} ${Math.round(
